...
from werkzeug.utils import secure_filename # Adicionado para upload de foto

# Imports de Módulos Locais
from models import db, Turma, Aluno, Atividade, Lembrete, Horario, BlocoAula, Presenca
from forms import TurmaForm, LembreteForm, UserProfileForm # Adicionado UserProfileForm

from flask_login import login_required, current_user

# Criação do Blueprint para Rotas Principais/Gerais
...
@login_required 
def index():
    # Removida a inicialização e o processamento do TurmaForm
    lembrete_form = LembreteForm(prefix='lembrete')

    # Removido o bloco de processamento do TurmaForm
    # O processamento do LembreteForm deve ser atualizado para usar url_for correto
...
# NOVO: Rota para adicionar turma
...
# --- ROTAS DE DASHBOARD E LEMBRETES GLOBAIS ---
...
def dashboard_global():
    ...
    # CORREÇÃO: Combina os dados de Desempenho e Frequência em Python (Correção do filtro 'merge')
    dados_combinados = {}
    ...
    # Converte o dicionário de volta para uma lista de objetos para o template
    dados_graficos = [
        {"turma": nome, **dados} 
        for nome, dados in dados_combinados.items()
    ]
    
    # NOVO: Lógica Top Alunos (copiado do passo anterior)
    top_alunos_data = []
    ...
    return render_template('dashboard_global.html',
                           ...
                           dados_combinados=dados_graficos, # Envia o objeto combinado
                           top_alunos=top_alunos_data
                           )

# --- ROTAS DE PERFIL DE USUÁRIO (NOVO) ---
@core_bp.route('/perfil/editar', methods=['GET', 'POST'])
@login_required
def edit_perfil():
    
    # Inicializa o formulário, passando o username atual para validação
    form = UserProfileForm(original_username=current_user.username)
    
    # Se o formulário não for válido, preenche com os dados do usuário
    ...
    if form.validate_on_submit():
        ...
        if foto_upload and foto_upload.filename != '':
            ...
            # Se já houver uma foto antiga, deleta ela
            ...
            # Garante que o stream de upload seja reiniciado/copiado se necessário
