...
# ------------------- FUNÇÕES HELPER LOCAIS (Necessárias para IA/Export) -------------------
...
# --- ROTA DELETAR ATIVIDADE (ADICIONADA para uso em edit_atividade.html) ---
...
def delete_atividade(atividade_id):
    ...
    try:
        # A lógica cascade deve deletar Presencas relacionadas, mas a exclusão explícita é mais segura.
        # Devido ao cascade no modelo Presenca, vamos focar na Atividade:
        ...
        flash(f'Atividade "{atividade.titulo}" e seus registros foram deletados com sucesso.', 'success')
...
def gerar_prova_docx():
    ...
    # Cálculo da Média da Turma (função local/assumida em utils.py)
    ...
    # 2b. Dados do DB (Atividades) - MODIFICADO para ler anexos
    if atividades_ids:
    ...
    # 2c. Dados de Ficheiros (Upload) - Lendo da memória (I/O Otimizado)
    if fontes_externas:
